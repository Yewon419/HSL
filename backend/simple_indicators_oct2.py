...
def main():
    ...
    # Get all tickers with data on Oct 2, 2025
    with engine.connect() as conn:
        result = conn.execute(text("""
            SELECT DISTINCT s.ticker FROM stocks s
            JOIN stock_prices sp ON s.ticker = sp.ticker
            WHERE s.currency = 'KRW' AND sp.date = '2025-10-02'
            ORDER BY s.ticker
        """))
        tickers = [row[0] for row in result]

    logger.info(f"Processing {len(tickers)} Korean stocks for Oct 2, 2025")
    ...
    for i, ticker in enumerate(tickers):
        try:
            ...
            with engine.connect() as conn:
                ...
                if len(df) < 20:  # Need at least 20 days
                    continue
                ...
                indicators = {
                    ...
                    'ma_200': float(ma200.iloc[latest_idx]) if pd.notna(ma200.iloc[latest_idx]) else None,
                ...
                if success_count % 100 == 0:
                    logger.info(f"Progress: {success_count}/{len(tickers)} tickers")
