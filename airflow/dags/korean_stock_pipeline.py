...
dag = DAG(
    ...
    description='Korean stock data collection using web scraping',
    schedule_interval='0 19 * * 1-5',  # 매일 오후 7시 (한국 시장 종료 후)
    catchup=True,  # 누락된 실행을 따라잡도록 설정
    max_active_runs=1,
    tags=['korean-stock', 'daily', 'etl']
)
...
def generate_technical_indicators(**context):
    """종합 기술적 지표 생성 (32개 지표)"""
    logging.info("Starting comprehensive technical indicators generation")

    try:
        # comprehensive_indicators.py 모듈 임포트 (같은 디렉토리에 있음)
        import sys
        import os
        ...
        target_date = context['ds']  # YYYY-MM-DD format
        logging.info(f"Generating 32 comprehensive indicators for date: {target_date}")
        ...
        logging.info(f"Generating comprehensive indicators for {len(tickers)} Korean stocks")

        # 종합 지표 계산 실행 (날짜 기반)
        success_count = calculate_comprehensive_indicators(target_date)

        logging.info(f"Comprehensive technical indicators generation completed: {success_count}/{len(tickers)} successful")
        return success_count

    except Exception as e:
        logging.error(f"Comprehensive technical indicators generation failed: {e}")
        import traceback
        traceback.print_exc()

        # 실패 시 기존 기본 지표라도 계산
        logging.warning("Falling back to basic indicators calculation")
...
def sync_indicators_to_grafana(**context):
    """기술적 지표를 Grafana 형식으로 동기화 (종합 지표 지원)"""
    logging.info("Starting sync from comprehensive_indicators/technical_indicators to indicator_values")

    try:
        # sync_indicators_to_grafana.py 스크립트 실행
        import subprocess
        import sys

        dags_dir = os.path.dirname(os.path.abspath(__file__))
        script_path = os.path.join(dags_dir, "sync_indicators_to_grafana.py")
        ...
        if result.returncode == 0:
            logging.info(f"Successfully executed sync script: {result.stdout}")

            # 결과에서 성공 카운트 추출
            import re
            match = re.search(r'Sync completed: (\d+) records processed', result.stdout)
            success_count = int(match.group(1)) if match else 0

            logging.info(f"Grafana sync completed: {success_count} records processed")
            return success_count
        else:
            logging.error(f"Sync script failed: {result.stderr}")
            # 실패 시 기본 동기화 시도
            return sync_basic_indicators_to_grafana(context)

    except Exception as e:
        logging.error(f"Sync script execution failed: {e}")
        # 실패 시 기본 동기화 시도
        return sync_basic_indicators_to_grafana(context)
...
fetch_korean_data_task = PythonOperator(
    ...
    dag=dag,
    execution_timeout=timedelta(hours=3),  # 3시간 타임아웃 (전종목 수집용)
    retries=0,  # 재시도 안 함 (너무 오래 걸림)
)
