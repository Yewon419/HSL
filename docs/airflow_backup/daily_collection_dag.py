...
매일 주가 데이터와 시장 지수를 자동으로 수집하는 DAG

스케줄: 매일 08:00 UTC (한국 시간 17:00 - 오후 5시, 애프터마켓 마감 후)
특징:
- 매일 자동으로 Task Job 생성
...
dag = DAG(
    ...
    description='매일 주가 데이터와 시장 지수를 자동으로 수집',
    schedule_interval='0 8 * * 1-5',  # 평일(월~금) 08:00 UTC (한국 시간 17:00 - 오후 5시) - 주말/공휴일 제외
    catchup=False,  # 과거 날짜는 수동으로 generate_task_dag 사용
    max_active_runs=1,  # 동시 실행 최대 1개
    tags=['production', 'daily', 'collection', 'stock-prices', 'market-indices']
)
...
def task_fetch_stock_prices(**context):
    ...
    logger.info(f"[daily_collection_dag] Fetching stock prices for {target_date}")
...
def task_fetch_market_indices(**context):
    ...
    logger.info(f"[daily_collection_dag] Fetching market indices for {target_date}")
...
def task_save_to_database(**context):
    ...
    logger.info(f"[daily_collection_dag] Saving data to database for {target_date}")
