...
def collect_fundamentals(**context):
    """펀더멘탈 데이터 수집 (PER, PBR, EPS, BPS 등)"""
    execution_date = context['execution_date']
    target_date = execution_date.date()

    logging.info(f"Collecting fundamental data for {target_date}")
    ...
    try:
        ...
        logging.info(f"Processing fundamentals for {len(stocks)} stocks")
        ...
        for stock in stocks:
            ...
            try:
                ...
                error_count += 1
                if error_count <= 5:
                    logging.warning(f"Error collecting fundamentals for {ticker}: {e}")
                continue

        logging.info(f"Fundamentals collected: {success_count} success, {error_count} errors")
        return {'success': success_count, 'errors': error_count}

    except Exception as e:
        logging.error(f"Fatal error in fundamentals collection: {e}")
        raise
...
def validate_data_quality(**context):
    ...
    try:
        with engine.connect() as conn:
            ...
            # Check fundamentals
            fundamental_result = conn.execute(text("""
                SELECT COUNT(*) as count,
                ...
                WHERE date = :date
            """), {'date': target_date})
            fund_row = fundamental_result.fetchone()
            fundamental_count = fund_row[0]
            per_count = fund_row[1]
            pbr_count = fund_row[2]
            ...
            logging.info(f"  - Fundamentals: {fundamental_count} (PER: {per_count}, PBR: {pbr_count})")

            # Validation criteria
            passed = True
            if price_count < 100:
                logging.warning(f"⚠️ Stock prices count too low: {price_count}")
                passed = False
            if indicator_count < 100:
                logging.warning(f"⚠️ Technical indicators count too low: {indicator_count}")
                passed = False
            if fundamental_count < 50:
                logging.warning(f"⚠️ Fundamentals count too low: {fundamental_count}")
                passed = False
            ...
            if passed:
                ...
                logging.warning("⚠️ Data quality validation FAILED")
                return False
...
collect_fundamentals_task = PythonOperator(
    task_id='collect_fundamentals',
    python_callable=collect_fundamentals,
    dag=dag,
)
...
# 1. 주가와 시장지수를 먼저 수집
# 2. 주가 기반으로 기술적 지표 계산과 펀더멘탈 수집을 병렬로 실행
# 3. 모두 완료 후 검증
collect_prices_task >> [calculate_indicators_task, collect_fundamentals_task]
collect_indices_task >> validate_task
[calculate_indicators_task, collect_fundamentals_task] >> validate_task
