...
def collect_market_indices():
    ...
    logger.info("=" * 80)
    logger.info(f"시장 지수 수집 시작: {TARGET_DATE}")
    logger.info("=" * 80)
    ...
    try:
        ...
        for index_name in indices:
            try:
                df = pykrx_stock.get_market_index_ohlcv(DATE_STR, DATE_STR, index_name)
                if not df.empty:
                    for date_index, row in df.iterrows():
                        db.execute(text("""
                            INSERT INTO market_indices
                            (index_name, date, open_value, high_value, low_value, close_value, volume, created_at)
                            VALUES (:index_name, :date, :open_value, :high_value, :low_value, :close_value, :volume, :created_at)
                    ...
                    saved_count += 1
                    logger.info(f"  {index_name}: 저장됨")
            except Exception as e:
                logger.warning(f"  {index_name} 오류: {e}")
                continue

        db.commit()
        logger.info(f"✅ 시장 지수 수집 완료: {saved_count} 개 인덱스 저장됨")

        return saved_count

    except Exception as e:
        logger.error(f"시장 지수 수집 중 오류: {e}", exc_info=True)
        raise
    finally:
        db.close()
    ...
    for i, indicators in enumerate(indicators_list):
        try:
            db.execute(text("""
                ...
                (ticker, date, rsi, macd, macd_signal, sma_20, sma_50, sma_200, bb_upper, bb_lower, created_at)
                VALUES (:ticker, :date, :rsi, :macd, :macd_signal, :sma_20, :sma_50, :sma_200, :bb_upper, :bb_lower, :created_at)
                ON CONFLICT (ticker, date) DO UPDATE SET
                    rsi = EXCLUDED.rsi,
                    macd = EXCLUDED.macd,
...
if __name__ == '__main__':
    try:
        logger.info(f"\n{'='*80}")
        logger.info(f"2025-10-22 주가, 지수 수집 및 기술지표 계산")
        logger.info(f"{'='*80}\n")
        ...
        # 2. 지수 수집
        indices_count = collect_market_indices()
        logger.info("")
        ...
        logger.info(f"  - 지수: {indices_count} 인덱스 저장")
