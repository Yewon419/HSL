#!/usr/bin/env python3
"""
2022년 데이터가 없는 종목들의 2022년도 데이터만 수집하는 스크립트
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('collect_missing_2022.log'),
...
def get_stocks_missing_2022_data():
    """2022년 데이터가 없는 종목 조회"""
...
def collect_2022_data_for_stock(ticker, company_name, market_type):
    """특정 종목의 2022년 데이터 수집"""
    engine = get_db_connection()
    ...
    try:
        ...
        logger.info(f"  PyKRX에서 2022년 데이터 가져오는 중...")

        # pykrx로 데이터 가져오기
        df = stock.get_market_ohlcv_by_date(start_date, end_date, ticker)

        if df is None or len(df) == 0:
            logger.warning(f"  2022년 데이터 없음")
            return 0
...
def main():
    """메인 함수"""
    logger.info("=" * 80)
    logger.info("2022년 데이터 누락 종목 수집 시작")
    logger.info("=" * 80)

    start_time = datetime.now()

    # 2022년 데이터가 없는 종목 조회
    logger.info("2022년 데이터가 없는 종목 조회 중...")
    stocks = get_stocks_missing_2022_data()
    total_stocks = len(stocks)

    logger.info(f"\n발견된 종목 수: {total_stocks}개")

    if total_stocks == 0:
        logger.info("모든 종목에 2022년 데이터가 있습니다!")
        return
    ...
    for i, stock_info in enumerate(stocks):
        ...
        try:
            ...
            saved_count = collect_2022_data_for_stock(ticker, company_name, market_type)
