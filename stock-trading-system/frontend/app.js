class StockTradingApp {
    ...
    async loadStocks() {
        ...
        try {
            const response = await fetch(`${this.baseURL}/v1/stocks?limit=3000`, {
                headers: {
                    'Authorization': `Bearer ${this.token}`
                }
            });
    ...
    async handleAddStock(e) {
        ...
        try {
            const response = await fetch(`${this.baseURL}/v1/stocks`, {
                method: 'POST',
                headers: {
    ...
    async loadChart() {
        ...
        try {
            const [priceResponse, indicatorResponse] = await Promise.all([
                fetch(`${this.baseURL}/v1/stocks/${ticker}/prices?limit=200`, {
    ...
    async loadTradingSignals() {
        ...
        document.getElementById('signals-actions-card').style.display = 'none';
    ...
    initNotificationHandlers() {
        ...
        // 이메일 전송 버튼
        document.getElementById('send-signals-email-btn').addEventListener('click', () => {
            this.sendSignalsViaEmail();
        });

        // Slack 전송 버튼
        document.getElementById('send-signals-slack-btn').addEventListener('click', () => {
            this.sendSignalsViaSlack();
        });
    ...
    saveNotificationSettings() {
        ...
        const slackWebhook = document.getElementById('notification-slack-webhook').value;

        // 유효성 검사
        if (email && !this.isValidEmail(email)) {
            this.showNotification('유효한 이메일 주소를 입력하세요', 'warning');
            return;
        }

        if (slackWebhook && !this.isValidSlackWebhook(slackWebhook)) {
            this.showNotification('유효한 Slack 웹훅 URL을 입력하세요', 'warning');
            return;
        }
    ...
    loadNotificationSettings() {
        ...
        if (saved) {
            ...
            if (settings.slackWebhook) {
                document.getElementById('notification-slack-webhook').value = settings.slackWebhook;
            }
        }
    }
    ...
    async sendSignalsViaEmail() {
        ...
        if (!email) {
            this.showNotification('먼저 알림 설정에서 이메일을 입력하세요', 'warning');
            return;
        }

        // 현재 필터된 신호 가져오기
        const signals = this.getCurrentFilteredSignals();

        if (signals.length === 0) {
            this.showNotification('전송할 매매 신호가 없습니다', 'warning');
            return;
        }
        ...
        try {
            ...
            const dateFilter = document.getElementById('signal-date-filter').value || '2025-10-02';

            const response = await fetch('http://localhost:8000/api/trading/notifications/send-signals', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (response.ok && data.status === 'success') {
                this.showNotification(`${signals.length}개의 매매 신호를 이메일로 전송했습니다`, 'success');
            } else {
                this.showNotification(data.message || '이메일 전송 중 오류가 발생했습니다', 'error');
            }
        } catch (error) {
    ...
    async sendSignalsViaSlack() {
        ...
        if (!slackWebhook) {
            this.showNotification('먼저 알림 설정에서 Slack 웹훅을 입력하세요', 'warning');
            return;
        }
        ...
        try {
            ...
            if (response.ok && data.status === 'success') {
                this.showNotification(`${signals.length}개의 매매 신호를 Slack으로 전송했습니다`, 'success');
            } else {
                this.showNotification(data.message || 'Slack 전송 중 오류가 발생했습니다', 'error');
            }
        } catch (error) {
            console.error('Slack 전송 오류:', error);
            this.showNotification('Slack 전송 중 오류가 발생했습니다', 'error');
        } finally {
            this.showLoading('signals-loading', false);
        }
    }

    getCurrentFilteredSignals() {
        const tbody = document.getElementById('signals-grid-body');
        const rows = tbody.querySelectorAll('tr');

        // 필터된 신호 수 반환 (헤더 제외)
        return Array.from(rows).filter(row => row.cells.length > 1);
    }
