#!/usr/bin/env python3
"""
2025년 1월부터 현재까지 데이터가 부족한 모든 종목 수집
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('collect_2025_all_stocks.log'),
...
def get_stocks_with_insufficient_2025_data():
    """2025년 데이터가 부족한 종목 조회 (예상 거래일의 50% 미만)"""
    engine = get_db_connection()

    try:
        # 2025년 1월 1일부터 오늘까지 예상 거래일 수 (대략 270일 중 180일 정도)
        # 50% 미만인 종목 찾기
        with engine.connect() as conn:
            result = conn.execute(text("""
                ...
                HAVING COUNT(sp.date) < 100  -- 100일 미만인 종목
...
def collect_2025_data_for_stock(ticker, company_name, market_type):
    """특정 종목의 2025년 데이터 수집"""
    engine = get_db_connection()
    ...
    try:
        ...
        logger.info(f"  PyKRX에서 2025년 데이터 가져오는 중...")

        # pykrx로 데이터 가져오기
        df = stock.get_market_ohlcv_by_date(start_date, today, ticker)

        if df is None or len(df) == 0:
            logger.warning(f"  2025년 데이터 없음")
            return 0
...
def main():
    """메인 함수"""
    logger.info("=" * 80)
    logger.info("2025년 데이터 부족 종목 수집 시작 (1월~현재)")
    logger.info("=" * 80)

    start_time = datetime.now()

    # 2025년 데이터가 부족한 종목 조회
    logger.info("2025년 데이터가 부족한 종목 조회 중...")
    stocks = get_stocks_with_insufficient_2025_data()
    total_stocks = len(stocks)

    logger.info(f"\n발견된 종목 수: {total_stocks}개")

    if total_stocks == 0:
        logger.info("모든 종목에 충분한 2025년 데이터가 있습니다!")
        return
    ...
    for i, stock_info in enumerate(stocks):
        ...
        current_records = stock_info['current_records']
        ...
        try:
            ...
            logger.info(f"현재 레코드: {current_records}개")
            logger.info(f"{'='*80}")

            # 데이터 수집 시작
            stock_start = datetime.now()

            saved_count = collect_2025_data_for_stock(ticker, company_name, market_type)
