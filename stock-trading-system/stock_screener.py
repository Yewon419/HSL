...
class StockScreener:
    ...
    def _apply_single_condition(self, condition: Dict, date_range: Tuple[str, str]) -> pd.DataFrame:
        ...
        elif condition_type == 'bb_breakout_ma_touch':
            return self._find_bb_breakout_ma_touch(date_range, condition.get('lookback_days', 5), condition.get('ma_period', 20))
        else:
    ...
    def _find_bb_breakout_ma_touch(self, date_range: Tuple[str, str], lookback_days: int = 5, ma_period: int = 20) -> pd.DataFrame:
        """
        볼린저밴드 하단 돌파 후 이동평균선 터치 전략

        조건:
        1. 종가가 볼린저밴드 하단선을 하향 돌파했다가
        2. 다시 상승하여 이동평균선(MA)을 터치하거나 돌파한 경우

        Args:
            date_range: 분석 기간
            lookback_days: 패턴 확인 기간 (기본 5일)
            ma_period: 이동평균 기간 (기본 20일)

        Returns:
            pd.DataFrame: 조건을 만족하는 종목과 날짜
        """
        try:
            # 볼린저밴드의 중간선(bb_middle)을 이동평균으로 사용하는 간단한 버전
            query = """
            WITH bb_data AS (
                SELECT
                    iv_bb.ticker,
                    iv_bb.date,
                    sp.close_price,
                    (iv_bb.value::json->>'bb_lower')::float as bb_lower,
                    (iv_bb.value::json->>'bb_upper')::float as bb_upper,
                    (iv_bb.value::json->>'bb_middle')::float as bb_middle,
                    LAG(sp.close_price, 1) OVER (PARTITION BY iv_bb.ticker ORDER BY iv_bb.date) as prev_close,
                    LAG((iv_bb.value::json->>'bb_lower')::float, 1) OVER (PARTITION BY iv_bb.ticker ORDER BY iv_bb.date) as prev_bb_lower,
                    LAG(sp.close_price, 2) OVER (PARTITION BY iv_bb.ticker ORDER BY iv_bb.date) as prev2_close,
                    LAG((iv_bb.value::json->>'bb_lower')::float, 2) OVER (PARTITION BY iv_bb.ticker ORDER BY iv_bb.date) as prev2_bb_lower
                FROM indicator_values iv_bb
                JOIN indicator_definitions id_bb ON iv_bb.indicator_id = id_bb.id AND id_bb.code = 'BB'
                JOIN stock_prices sp ON iv_bb.ticker = sp.ticker AND iv_bb.date = sp.date
                WHERE iv_bb.date BETWEEN :start_date AND :end_date
                AND (iv_bb.value::json->>'bb_lower') IS NOT NULL
                AND (iv_bb.value::json->>'bb_middle') IS NOT NULL
            ),
            pattern_detection AS (
                SELECT
                    ...
                    prev2_bb_lower,
                    CASE
                        -- 조건 1: 이전에 BB 하단 돌파가 있었는지 확인
                        WHEN prev2_close > prev2_bb_lower AND prev_close < prev_bb_lower THEN true
                        ELSE false
                    END as had_bb_breakout,
                    CASE
                        -- 조건 2: 현재 BB 중간선(이동평균)을 터치하거나 돌파했는지 확인
                        WHEN close_price >= bb_middle * 0.99 THEN true  -- 1% 오차 허용
                        ELSE false
                    END as touching_ma,
                    CASE
                        -- 추가 필터: 상승 모멘텀 확인
                        WHEN close_price > prev_close THEN true
                        ELSE false
                    END as upward_momentum
                FROM bb_data
            ...
            SELECT
                ...
                bb_lower,
                bb_middle,
                bb_middle as sma_20,  -- BB 중간선을 이동평균으로 사용
                ROUND(CAST(((close_price - bb_lower) / bb_lower * 100) AS NUMERIC), 2) as bb_lower_distance_pct,
                ROUND(CAST(((close_price - bb_middle) / bb_middle * 100) AS NUMERIC), 2) as ma_distance_pct
            FROM pattern_detection
            WHERE had_bb_breakout = true
            AND touching_ma = true
            AND upward_momentum = true
            ORDER BY date DESC, ticker
            """
            ...
            with self.engine.connect() as conn:
                ...
                columns = [
                    'ticker', 'date', 'current_price', 'bb_lower', 'bb_middle',
                    f'sma_{ma_period}', 'bb_lower_distance_pct', 'ma_distance_pct'
                ]
                df = pd.DataFrame(result.fetchall(), columns=columns)

                logger.info(f"Found {len(df)} stocks matching BB breakout + MA touch pattern")
                return df

        except Exception as e:
            logger.error(f"Error finding BB breakout MA touch pattern: {e}")
            import traceback
            traceback.print_exc()
            return pd.DataFrame()
