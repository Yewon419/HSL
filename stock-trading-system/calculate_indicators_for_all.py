...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('indicators_calculation.log'),
...
def get_all_stocks_with_data():
    """데이터가 있는 모든 주식 목록 가져오기"""
    engine = get_db_connection()

    try:
        with engine.connect() as conn:
            result = conn.execute(text("""
                SELECT DISTINCT s.ticker, s.company_name, s.market_type, COUNT(sp.date) as data_count
                FROM stocks s
                ...
                HAVING COUNT(sp.date) >= 50  -- 최소 50일 데이터가 있는 종목만
...
def calculate_indicators_for_stock(ticker, start_date=None, end_date=None):
    """특정 종목의 기술적 지표 계산"""
    engine = get_db_connection()
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            price_data = pd.read_sql(text(query), conn, params=params)
...
def calculate_all_indicators():
    ...
    logger.info(f"Found {total_stocks} stocks with sufficient data")
    ...
    for i, stock in enumerate(stocks):
        ...
        try:
            logger.info(f"[{i+1}/{total_stocks}] Processing {ticker} ({company_name}) - {market_type} ({data_count} records)")
            ...
            if saved_count > 0:
                ...
                logger.info(f"  -> Calculated {saved_count} indicators for {ticker}")
            else:
                failed_count += 1
