...
async def save_signals_bulk(request: BulkSignalsRequest):
    ...
    스크리닝 탭에서 전략 실행 시 발견된 종목들을 매매신호 탭에 저장
    """
    try:
        from sqlalchemy import create_engine
        from sqlalchemy.orm import sessionmaker
        from trading_service.models import TradingSignal, SignalType, SignalStatus
        ...
        for signal_data in request.signals:
            try:
                ...
                if existing:
                    logger.info(f"Signal already exists for {signal_data.ticker} on {signal_date}")
                    continue
                ...
                logger.error(f"Error saving signal for {signal_data.ticker}: {e}")
                continue
