...
매일 07:30 UTC (오후 4시 30분 KST)에 실행되어 주가/지수를 수집합니다.
- 정해진 시간에 한 번만 실행
- 스케줄: 30 7 * * 1-5 (매일 07:30 UTC, 평일만)
- UTC 07:30 = 한국시간 오후 4시 30분 (KST+9)
"""
...
default_args = {
    ...
    'start_date': datetime(2025, 11, 10),  # 오늘 날짜로 설정 (catchup=False와 함께 사용)
...
dag = DAG(
    ...
    description='Daily collect stock prices and market indices at 07:30 UTC (16:30 KST)',
    schedule_interval='30 7 * * 1-5',  # 매일 07:30 UTC (한국시간 16시 30분, 평일만)
    catchup=False,
...
def check_and_decide_collection(**context):
    ...
    try:
        # 16:30(한국시간 오후 4시 30분) 이전이면 스킵
        current_time_minutes = current_hour * 60 + current_minute
        collection_time_minutes = 16 * 60 + 30  # 16:30 KST
        if current_time_minutes < collection_time_minutes:
            logger.info(f"Skipping: current time ({current_hour}:{current_minute:02d} KST) is before 16:30 KST")
...
# Task 의존성: check_task 완료 후 병렬로 price/indices 수집 → 완료 후 지표계산 DAG trigger
